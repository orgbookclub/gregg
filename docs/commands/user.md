...
| readerboard [preset] [from] [to]  | Shows the server reading leaderboard. Optional `preset` (`This Year` / `Last Year` / `This Month`) or a custom `from`/`to` range (YYYY-MM-DD, inclusive) — filters by event end date. Defaults to All Time. `preset` and `from`/`to` are mutually exclusive |
