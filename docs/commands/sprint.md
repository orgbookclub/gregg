...
| stats [user] [preset] [from] [to] | Shows the total sprint stats of a user. Optional `preset` (`This Year` / `Last Year` / `This Month`) or a custom `from`/`to` range (YYYY-MM-DD, inclusive) filters by sprint end date. Defaults to All Time. Surfaces sprint count + completion rate, total pages, total time + average pace, personal records (best sprint by pages, fastest sprint by pages/min), and a weekly streak (longest, plus current when the window includes this week) |
